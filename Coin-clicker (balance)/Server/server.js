...
const users = [];
let lastUserId = 0;
const { encodePassword, comparePassword } = require('./hash');
...
app.post('/sign-up', (req, res) => {
  ...
  lastUserId += 1; 
  const id = lastUserId;
  users.push({ id, email, password: encodePassword(password) });
  res.status(201).json({ message: 'Реєстрація успішна!', email, id });
});
